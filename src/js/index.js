...
const controlCountries = async () => {
    ...
    try{
        ...
        CountriesView.renderCountry(state.confirmedTotal,state.recoveredTotal,state.deathsTotal);
...
document.querySelector('.pagination').addEventListener('click', (e) =>  {
    ...
    if(slide === "next" && count < 19){
        prev.dataset.count++;
        next.dataset.count++;
        CountriesView.clearTable();
        CountriesView.renderCountry(state.confirmedTotal,state.recoveredTotal,state.deathsTotal,count,prev.dataset.count,next.dataset.count);
    }
...
const controlAsean = async () => {
    ...
    try{
        ...
        state.asean = [state.myanmar,state.vietnam,state.thailand,state.brunei,state.cambodia,state.timor,
            state.indonesia,state.laos,state.malaysia,state.philippines,state.singapore];
